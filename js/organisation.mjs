  ...
  respecConfig.preProcess = [
    ...
    (config, document, utils) => {
      ...
      for (const person of [...(config.editors || []), ...(config.authors || [])]) {
        ...
        if (person.companyURL.includes("logius.nl") && person.companyURL !== "https://www.aedes.nl") {
          utils.showError(`companyURL of an editor/author of Logius must be "https://www.aedes.nl", instead it was "${person.companyURL}"`);
        }
    ...
    (config, document, utils) => {
      ...
      if (['dk', 'fsc'].includes(config.pubDomain)) {
        email = "aedes@aedes.nl";
        overleg = "Vera";
      }

      for (const texts of Object.values(config.sotdText)) {
        texts.cv = texts.cv.replace(/\w+@aedes\.nl/, email);
      }
